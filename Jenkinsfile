...
def slackNotifier(String buildResult) {
  if ( buildResult == "SUCCESS" ) {
    ...
    teamDomain: 'benbeneficios',
    tokenCredentialId: 'ben-slack',
    ...
    slackSend( teamDomain: 'benbeneficios',
      tokenCredentialId: 'ben-slack',
      channel: '#financeiro_contabil-build',
      message: "Job: ${env.JOB_NAME} \nBuild: ${env.BUILD_NUMBER} \nStatus: Abortado \nTempo: ${currentBuild.durationString.minus(' and counting')}")
  }
...
def configFromBranch(branch) {
    ...
    def env_staging = [
        rancherContext: 'c-lh4tc:p-58j5f',
        deploymentFile: 'deployment.yml',
        namespace: 'financeiro-contabil-stg'
    ]
    def env_prd = [
        rancherContext: '',
        deploymentFile: '',
...
pipeline {
    ...
    environment {
        ...
        GIT_URL="git@bitbucket.org:benvisavale/ctb-wkr-${workerName}.git"
        PROJECT_NAME="ctb-wkr-${workerName}"
